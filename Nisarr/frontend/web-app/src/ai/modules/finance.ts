...
For ADD_TO_SAVINGS, data must include: name (string), amount (number to add). 
- If user doesn't specify a name, but context shows they only have ONE savings goal, use that name.
- If user has multiple goals and doesn't specify, use action: "CHAT" to ask which one.
...
- "add 900 tk savings" (if only one goal exists, e.g. "Savings") → ADD_TO_SAVINGS with name "Savings", amount 900
...
const parseAmount = (val: any): number => {
    ...
    if (typeof val === 'string') {
        // Remove currency symbols and comma, then parse
        const cleaned = val.replace(/[৳$, tk, taka]/gi, '').replace(/,/g, '');
    ...
    switch (action) {
        ...
        case "EDIT_INCOME":
            await hooks.updateEntry.mutateAsync({
                id: String(data.id),
                amount: data.amount ? parseAmount(data.amount) : undefined,
        ...
        case "ADD_BUDGET":
            await hooks.addBudget.mutateAsync({
                ...
                type: "budget",
                target_amount: parseAmount(data.target_amount),
        ...
        case "UPDATE_BUDGET": {
            ...
            if (budgetToUpdate) {
                await hooks.updateBudget.mutateAsync({
                    ...
                    target_amount: data.target_amount ? parseAmount(data.target_amount) : undefined,
        ...
        case "UPDATE_SAVINGS": {
            ...
            if (savingsToUpdate) {
                await hooks.updateBudget.mutateAsync({
                    ...
                    target_amount: data.target_amount ? parseAmount(data.target_amount) : undefined,
                    current_amount: data.current_amount ? parseAmount(data.current_amount) : undefined,
                });
            }
            break;
        }
